...
impl HarnessAdapter for OpenCodeAdapter {
    ...
    /// Only the Codex subscription, and only in the login helper's own
    /// context. The Anthropic subscription has no OpenCode login at all — its
    /// OAuth plugin was removed upstream in 1.3.0 (finding 9) — and is the
    /// Claude adapter's `claude setup-token` to own.
    async fn login(
        &self,
        runner: &dyn Runner,
        provider: &str,
...
async fn login_flow(runner: &dyn Runner, name: &str) -> Result<LoginFlow, AdapterError> {
    ...
    tokio::spawn(async move {
        while let Ok(Some(line)) = lines.next_line().await {
            let mut sink = drain.lock().unwrap();
            sink.push_str(&line);
            sink.push('\n');
            if sink.len() > 64 * 1024 {
