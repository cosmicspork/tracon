...
use super::{omp::OmpAdapter, HarnessAdapter, HarnessEvent, LaunchSpec, PermissionReply};
use crate::runner::{local::LocalRunner, Runner, RunnerCommand};

/// The fake agent is a bin target of this crate, so cargo places it beside the
/// test executable (`target/<profile>/fake_agent`, tests live in `.../deps/`).
fn fake_agent_path() -> String {
    ...
    dir.join("fake_agent").to_string_lossy().into_owned()
}
...
impl Runner for FakeRunner {
    ...
    ) -> Result<tokio::process::Child, crate::runner::RunnerError> {
        cmd.argv = vec![fake_agent_path()];
...
async fn unknown_model_is_refused_before_prompting() {
    ...
    assert!(matches!(err, super::AdapterError::UnknownModel(m) if m == "m/nope"));
}
