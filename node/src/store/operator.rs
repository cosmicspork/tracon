...
impl OperatorQuestionRow {
    fn from_row(r: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            prompt: r.get("prompt")?,
            choices_json: r.get("choices_json")?,
            state: r.get("state")?,
...
impl Store {
    ...
    pub fn insert_operator_question(
        &self,
        row: &OperatorQuestionRow,
    ) -> Result<OperatorQuestionRow> {
    ...
    pub fn operator_question(&self, id: &str) -> Result<Option<OperatorQuestionRow>> {
        ...
        conn.query_row(
            "SELECT * FROM operator_question WHERE id=?1",
            [id],
            OperatorQuestionRow::from_row,
        )
        .optional()
    ...
    pub fn open_operator_questions(&self) -> Result<Vec<OperatorQuestionRow>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT * FROM operator_question WHERE state='unanswered' ORDER BY created_ms",
        )?;
        let rows = stmt
            .query_map([], OperatorQuestionRow::from_row)?
    ...
    pub fn issue_drafts(&self, open_only: bool) -> Result<Vec<IssueDraftRow>> {
        ...
        let sql = if open_only {
            "SELECT * FROM operator_issue WHERE state IN ('draft','publishing') ORDER BY created_ms"
        } else {
            "SELECT * FROM operator_issue ORDER BY created_ms DESC"
        };
        let mut stmt = conn.prepare(sql)?;
        let rows = stmt
            .query_map([], IssueDraftRow::from_row)?
            .collect::<std::result::Result<_, _>>()?;
        Ok(rows)
    }
    pub fn issue_draft(&self, id: &str) -> Result<Option<IssueDraftRow>> {
        let conn = self.conn.lock().unwrap();
        conn.query_row(
            "SELECT * FROM operator_issue WHERE id=?1",
            [id],
            IssueDraftRow::from_row,
        )
        .optional()
        .map_err(Into::into)
    }
    ...
    pub fn fail_issue_publication(&self, id: &str, error: &str) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute("UPDATE operator_issue SET state='draft', publish_error=?2 WHERE id=?1 AND state='publishing'", params![id,error])?;
        Ok(())
    }
    pub fn record_notification_attempt(
        ...
        let conn = self.conn.lock().unwrap();
        conn.execute("INSERT INTO operator_notification_attempt (id,notification_id,device_id,outcome,attempted_ms) VALUES (?1,?2,?3,?4,?5)", params![uuid::Uuid::now_v7().to_string(),notification_id,device_id,outcome,now_ms()])?;
        Ok(())
    }
    pub fn notification_attempts(
        &self,
        notification_id: &str,
    ) -> Result<Vec<NotificationAttemptRow>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt=conn.prepare("SELECT * FROM operator_notification_attempt WHERE notification_id=?1 ORDER BY attempted_ms")?;
        let rows = stmt
            .query_map([notification_id], |r| {
                Ok(NotificationAttemptRow {
                    id: r.get("id")?,
                    notification_id: r.get("notification_id")?,
                    device_id: r.get("device_id")?,
                    outcome: r.get("outcome")?,
                    attempted_ms: r.get("attempted_ms")?,
                })
            })?
            .collect::<std::result::Result<_, _>>()?;
        Ok(rows)
    ...
    pub fn claim_operator_notification(
        &self,
        dedup_key: &str,
        id: &str,
        ttl_ms: i64,
        ...
        let now = now_ms();
        conn.execute(
            "DELETE FROM operator_notification WHERE expires_ms <= ?1",
            [now],
        )?;
