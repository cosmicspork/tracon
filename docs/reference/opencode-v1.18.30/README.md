...
| Native UI route trace | `ui-route-trace.tsv`: 60 request shapes, captured by driving the served bundle in headless Chromium over CDP against the pinned binary behind the mediated gateway (`node/tests/opencode_route_trace.rs`, `TRACON_UI_TRACE=1`). 31 readable, 5 mediated (2 decided and forwarded, 2 refused, 1 answered 409 because the tour's session has no supervising actor), 2 mediated-but-unavailable, 8 forbidden, 7 asset, 2 page, 1 boot, and 4 deliberate 404s. **No route was unplaced**: every path the app used is either a route `http::ui`'s app-route table declares or a row the gateway's matrix classifies, and the check re-derives every recorded class from those two functions in CI without a browser. Finding 20 below is what the trace exposed |
