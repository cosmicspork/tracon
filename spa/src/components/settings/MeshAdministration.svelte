...
<div class="mesh-admin">
  <div class="heading">
    <span>Mesh administration</span>
  ...
  {#if !data}
    <p class="dim">Load administrator access to read the local membership snapshot.</p>
  {:else if !data.capabilities.invite}
  ...
  {:else}
    <p class="source">{data.inventory_source}</p>

    <section>
      <h3>Add a member</h3>
      <p>Select the existing channels this invitation may hand to the new member. The mesh coordination channel is included by the node and is not selectable here.</p>
    ...
    <section>
      <h3>Members and compatibility</h3>
    ...
    <section>
      <h3>Share an existing channel with the hub</h3>
      <p>The hub replica receives the selected channel keys for its declared processing role. This enables its optional shared-channel aggregation; it does not make it a member that can run work.</p>
      <div class="choices">
        {#each availableChannels as channel (channel.name)}
...
<style>
  .mesh-admin { display: grid; gap: 14px; min-width: 0; overflow-wrap: anywhere; }
  .heading { display: flex; align-items: baseline; gap: 12px; flex-wrap: wrap; font-weight: 600; }
  section > p { margin: 0; color: var(--ink2); max-width: 72ch; }
  ...
  .invite, article { display: grid; gap: 7px; min-width: 0; background: var(--s1); border-radius: 4px; padding: 11px 12px; }
  ...
  .url input { min-height: 40px; width: 100%; background: var(--s2); border: 1px solid var(--rule); border-radius: 4px; color: var(--ink2); padding: 7px 9px; font: 12px var(--mono); }
  ...
  .empty { padding: 14px; background: var(--s1); border-radius: 4px; color: var(--dim); }
